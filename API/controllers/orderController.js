...
// ── Fetch SKU for a product id ────────────────────────────────────────────────
async function getProductSku(productId) {
    try {
        const [rows] = await db.execute('SELECT sku FROM products WHERE id = ? LIMIT 1', [productId]);
...
async function pushToShipeaso(orderId, order, items) {
    ...
    try {
        ...
        // Build line_items — fetch SKU from products table for each item
        const lineItems = await Promise.all(items.map(async (item) => {
            let skuCode = item.sku || null;
            if (!skuCode && item.id) {
                skuCode = await getProductSku(item.id);
            }
            skuCode = skuCode || String(item.id);
            return {
                sku_code:       skuCode,
        ...
        await saveShipeasoResponse(orderId, JSON.stringify(response.data));
        ...
        try { await saveShipeasoResponse(orderId, JSON.stringify({ error: errData })); } catch {}
    }
}
